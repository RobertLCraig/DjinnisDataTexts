...
local function GetLine(parent, index)
    ...
    -- Durability column (far right, width from font metrics)
    ...
    durability:SetText("100%")
    durability:SetWidth(math.ceil(durability:GetStringWidth()) + 6)
    durability:SetText("")

    -- iLvl value (left of durability, width from font metrics)
    local value = ns.FontString(frame, "DDTFontNormal")
    value:SetPoint("RIGHT", durability, "LEFT", -4, 0)
    value:SetJustifyH("RIGHT")
    value:SetWordWrap(false)
    value:SetText("000")
    value:SetWidth(math.ceil(value:GetStringWidth()) + 6)
    value:SetText("")

    -- Status indicators (anchored left of value)
    local status = ns.FontString(frame, "DDTFontSmall")
    status:SetPoint("RIGHT", value, "LEFT", -4, 0)
    status:SetJustifyH("RIGHT")
    status:SetWordWrap(false)

    -- Slot name label (fills remaining space)
